...
/*   Updated: 2024/08/24 00:37:37 by mtelek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	close_failed(t_main *main, int fd)
{
	if (errno == EBADF)
		...
		ft_putstrs_fd(E_CLOSE_FD, strerror(errno), NULL, fd);
	free_structs(main);
}
...
{
	if (errno == EACCES)
		ft_putstrs_fd(E_PERMISSION_D_OPEN, file_name, NULL, 2);
	else if (errno == ENOENT)
		ft_putstrs_fd(E_NO_FILE, file_name, NULL, 2);
	else if (errno == EROFS)
		ft_putstrs_fd(E_READ_ONLY, file_name, NULL, 2);
	else
